...
enyo.kind({
	...
	_create: function(inParam) {
		if(typeof Masonry === 'undefined') {
			__masonry(window);
		}
		var opts = {isResizable: false};
		if(this.container.columnWidth) {
			opts.columnWidth = this.container.columnWidth;
		}
		if(this.container.gutterWidth) {
			opts.gutterWidth = this.container.gutterWidth;
		}
		if(this.container.isFitWidth) {
			opts.isFitWidth = this.container.isFitWidth;
		}
		if(this.container.isRTL) {
			opts.isRTL = this.container.isRTL;
		}
		this.wall = new Masonry(this.container.hasNode(), opts);
	}
});

// We can't call this during load because the DOM doesn't have a body yet!
function __masonry( window, undefined ) {
